...
const handleUserPromptsAbort = (promptsValue) => {
  if (typeof promptsValue === "undefined") {
    logger.log(
      "😲 Happy ESLint configurations installation aborted by the user."
    );
    process.exit(0);
  }
};
...
export const makeInstallCommand = async () => {
  ...
  program
    .description("install Happy ESLint configurations")
    ...
    .action(async ({ version, saveProd }) => {
      ...
      logger.log(
        "🎉 Happy ESLint configurations installation completed successfully!"
      );
    });
